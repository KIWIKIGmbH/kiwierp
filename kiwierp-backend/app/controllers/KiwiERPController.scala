...
trait KiwiERPController extends Controller {
  ...
  (val accessToken: AccessToken,
   private val req: Request[A]) extends WrappedRequest(req)

  object KiwiERPAction {

    def async[A](p: BodyParser[A])(block: Request[A] => Future[Result]): Action[A] =
      Action.async(p) { req =>
  ...
  object AuthorizedAction {
    ...
    def async[A](p: BodyParser[A])(block: AuthorizedRequest[A] => Future[Result]): Action[A] =
      KiwiERPAction.async(p) { req =>
        val authorize: String => Future[AccessToken] =
          (token) => AuthorizationContext(token, req.method, req.path)

        val result: AccessToken => Future[Result] =
          (accessToken) => block(new AuthorizedRequest(accessToken, req))
  ...
  object Page {
    ...
    def apply[M](searchFunc: Int => Future[List[M]])
                (implicit req: AuthorizedRequest[AnyContent]): Future[(List[M], Int)] = {
